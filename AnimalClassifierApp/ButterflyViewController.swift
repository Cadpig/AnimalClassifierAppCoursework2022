...
class ButterflyViewController: UIViewController, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    ...
    let model = ButterfliesClassifierFirst()
    
    @IBAction func PredictButterfly(_ sender: Any) {//предсказывает вид бабочки по фотографии, и вероятность того, что это именно этот вид, и выводит на экран
        ProceedButton.isHidden = true
        guard let pixelBuffer = buffer(from: ImageView.image!) else { return }
        guard let ButterfliesClassifierOutput = try? model.prediction(image: pixelBuffer) else {
    fatalError("Unexpected runtime error.")
}
        InfoLabel.isHidden = false
        let sortedProbas = ButterfliesClassifierOutput.classLabelProbs.sorted { $0.1 > $1.1 }
        InfoLabel.text = String(format: NSLocalizedString("butterflyPrediction %.1@ %.2@", comment: ""), ButterfliesClassifierOutput.classLabel, String(Int(Array(sortedProbas)[0].value * 100)))
...
func initButtons(){//функция, инициализирующая кнопки
    ...
    func initLabels(){//функция, инициализирующая лейблы
        HeadLabel.text = NSLocalizedString("butterflyPick", comment: "PickButterfly")
